...
//         if(!profile) {
//             return res.json({ success: false, message: "Profile nto found" })
//         }
...
//         if (!input || input.trim() === '') {
//             return res.json({success: true, users: [], message: 'Please enter a search term'})
//         }
...
//         // Fixed: Return users in 'users' field, not 'message'
//         res.json({success: true, users: filteredUsers, message: `Found ${filteredUsers.length} users`})
...
//         res.json({success: true, message: 'You are no longer following this user'}) // Fixed typo
...
//         if (connectionRequests.length >= 20) {
//             return res.json({success: false, message: 'You have sent more than 20 connection requests in the last 24 hours'}) // Fixed typo
//         }

//         // Check if users are already connected
//         const connection = await Connection.findOne({ // Fixed typo: was 'connecion'
//             $or: [
//                 { from_user_id: userId, to_user_id: id },
//                 { from_user_id: id, to_user_id: userId }
//             ]
...
//             return res.json({success: true, message: 'Connection request sent successfully'}) // Fixed typo
            
//         } else if(connection && connection.status === 'accepted') {
//             return res.json({success: false, message: 'You are already connected with this user'}) // Fixed typo
//         }
...
//         if(!connection) {
//             return res.json({success: false, message: 'Connection not found!'}) // Fixed typo
//         }
...
//         const profile = await User.findById(profileId)
//         if(!profile) {
//             return res.json({ success: false, message: "Profile not found" }) // Fixed typo
//         }
//         const posts = await Post.find({user: profileId}).populate('user')
...
// Get User Connections - COMPLETELY REWRITTEN
export const getUserConnections = async (req, res) => {
    try {
        const { userId } = req.auth()
        ...
        console.log('User connections array:', user.connections)

        // Get followers - populate the user details
        let followers = []
        if (user.followers && user.followers.length > 0) {
            followers = await User.find({ _id: { $in: user.followers } }).select('_id username full_name email bio profile_picture location')
        }

        // Get following - populate the user details  
        let following = []
        if (user.following && user.following.length > 0) {
            following = await User.find({ _id: { $in: user.following } }).select('_id username full_name email bio profile_picture location')
        }

        // Get connections - populate the user details
        let connections = []
        if (user.connections && user.connections.length > 0) {
            connections = await User.find({ _id: { $in: user.connections } }).select('_id username full_name email bio profile_picture location')
        }

        // Get pending connections - find all pending requests TO this user
        const pendingConnectionRequests = await Connection.find({ 
            to_user_id: userId, 
            status: 'pending' 
        }).populate('from_user_id', '_id username full_name email bio profile_picture location')

        const pendingConnections = pendingConnectionRequests.map(conn => conn.from_user_id).filter(user => user !== null)
...
export const acceptConnectionRequest = async (req, res) => {
    try {
        ...
        if(!connection) {
            return res.json({success: false, message: 'Connection request not found or already processed'})
        }
